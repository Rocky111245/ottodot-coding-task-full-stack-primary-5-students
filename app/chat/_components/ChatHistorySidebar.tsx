// app/components/ChatHistory/ChatHistorySidebar.tsx
'use client'
...
export default function ChatHistorySidebar({
    ...
    useEffect(() => {
        if (isOpen) {
            const saved = localStorage.getItem('session_ids')
            setSessionIds(saved ? JSON.parse(saved) : [])
        }
    }, [isOpen])
            ...
            <div className={`fixed right-0 top-0 h-full w-80 bg-white shadow-2xl z-50 flex flex-col transition-transform duration-300 ease-out ${
                ...
                <div className="p-4 border-b border-gray-200 flex justify-between items-center bg-gradient-to-r from-blue-600 to-purple-600 text-white">
                    <div>
                        ...
                        <p className="text-xs opacity-90">{sessionIds.length} problems solved</p>
                    </div>
                    <button
                        onClick={onClose}
                                ...
                                >
                                    <div className="flex items-center justify-between mb-1">
                                        <span className="text-sm font-medium text-gray-700">
                                            Problem #{sessionIds.length - index}
                                        </span>
